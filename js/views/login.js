...
(function(){
  ...
  MKR.portals.login = {
    async render(root){
      ...
      // Seeds this device with sample data and opens a portal. The cached profile
      // is only a UI hint: every cloud read/write is still re-checked by Row Level
      // Security against the real signed-in user, so a preview session can never
      // reach another venue's data — with no Supabase session, it reaches none.
      U.qsa('button[data-prev]',root).forEach(b=> b.onclick=async()=>{
        b.disabled=true; b.textContent='Opening…';
        try{ await MKR.auth.startPreview(b.dataset.prev); }
        catch(e){ showErr('Preview failed to start: '+e.message); b.disabled=false; }
      });
