...
(function(){
  ...
  const Auth = {
    ...
    // ---- Local preview (no cloud account) ----
    //  Opens a portal against this device's sample data so the app can be shown
    //  or demoed without a Supabase account. There is deliberately no `uid`, so
    //  there is no auth session: every cloud query runs as an anonymous user and
    //  Row Level Security refuses it. Preview therefore sees local seed data only
    //  and cannot read or write any real venue's records.
    isPreview(){ return !!(this._profile && this._profile._preview); },
    async startPreview(role){
      role = ['owner','manager','staff'].includes(role) ? role : 'owner';
      const DEMO = {owner:{id:'u_boss', name:'James Carter'},
      ...
      location.hash = `#/${role}/${MKR.portals[role].home}`;
      MKR.router.render();
      return this._profile;
    },
    exitPreview(){ this._profile=null; this._clearCache(); location.hash='#/login'; MKR.router.render(); },
